...
import { t as BrowserPerformanceMeasurement } from "./BrowserPerformanceMeasurement-CtWHITno.js";
...
*/
var PersistentCacheKeys = { ACTIVE_ACCOUNT_FILTERS: "active-account-filters" };
/**
...
function addBrokerParameters(parameters, brokerClientId, brokerRedirectUri) {
	if (!parameters.has("brk_client_id")) parameters.set(BROKER_CLIENT_ID, brokerClientId);
	if (!parameters.has("brk_redirect_uri")) parameters.set(BROKER_REDIRECT_URI, brokerRedirectUri);
}
/**
...
//#endregion
//#region node_modules/@azure/msal-common/dist/network/RequestThumbprint.mjs
/*! @azure/msal-common v15.17.0 2026-03-18 */
...
var AuthorizationCodeClient = class extends BaseClient {
	...
	*/
	async createTokenRequestBody(request) {
		...
		instrumentBrokerParams(parameters, request.correlationId, this.performanceClient);
		const configClaims = request.skipBrokerClaims && parameters.has("brk_client_id") ? void 0 : this.config.authOptions.clientCapabilities;
...
//#endregion
//#region node_modules/@azure/msal-common/dist/protocol/Authorize.mjs
/*! @azure/msal-common v15.17.0 2026-03-18 */
/**
...
function getStandardAuthorizeRequestParameters(authOptions, request, logger, performanceClient) {
	...
	const configClaims = request.skipBrokerClaims && parameters.has("brk_client_id") ? void 0 : authOptions.clientCapabilities;
...
function addError(error, logger, event, stackMaxSize = 5) {
	if (!(error instanceof Error)) {
		...
		if (!event.serverErrorNo && (error instanceof ServerError || error instanceof InteractionRequiredAuthError) && error.errorNo) event.serverErrorNo = error.errorNo;
		return;
...
var apiIdToName = (id) => {
	if (typeof id === "number" && id in ApiName) return ApiName[id];
	return "unknown";
};
var InteractionType;
...
async function generateEarKey() {
	...
	const jwk = {
		alg: "dir",
		kty: "oct",
		k: urlEncodeArr(new Uint8Array(key))
	};
...
async function decryptEarResponse(earJwk, earJwe) {
	...
	if (earJweParts.length !== 5) throw createBrowserAuthError(failedToDecryptEarResponse, "jwe_length");
...
function buildConfiguration({ auth: userInputAuth, cache: userInputCache, system: userInputSystem, telemetry: userInputTelemetry }, isBrowserEnvironment) {
	...
	if (userInputAuth?.protocolMode && userInputAuth.protocolMode === ProtocolMode.OIDC && providedSystemOptions?.allowPlatformBroker) throw createClientConfigurationError(cannotAllowPlatformBroker);
	return {
		auth: {
...
var version = "4.30.0";
//#endregion
//#region node_modules/@azure/msal-browser/dist/cache/CacheKeys.mjs
/*! @azure/msal-browser v4.30.0 2026-03-18 */
var PREFIX = "msal";
...
function getAccountKeysCacheKey(schema = 2) {
	if (schema < 1) return `${PREFIX}.${ACCOUNT_KEYS}`;
	return `${PREFIX}.${schema}.${ACCOUNT_KEYS}`;
}
function getTokenKeysCacheKey(clientId, schema = 2) {
...
var CryptoOps = class CryptoOps {
	...
	*/
	async removeTokenBindingKey(kid) {
		...
		if (await this.cache.containsKey(kid)) throw createClientAuthError(bindingKeyNotRemoved);
	}
	/**
...
var CookieStorage = class {
	...
	setItem(key, value, cookieLifeDays, secure = true, sameSite = SameSiteOptions.Lax) {
		...
		if (secure || sameSite === SameSiteOptions.None) cookieStr += "Secure;";
...
//#endregion
//#region node_modules/@azure/msal-browser/dist/cache/CacheHelpers.mjs
/*! @azure/msal-browser v4.30.0 2026-03-18 */
/**
...
//#endregion
//#region node_modules/@azure/msal-browser/dist/cache/EncryptedData.mjs
/*! @azure/msal-browser v4.30.0 2026-03-18 */
...
var LocalStorage = class {
	...
	async initialize(correlationId) {
		...
		if (parsedCookie.key && parsedCookie.id) {
			...
			cookies.setItem(ENCRYPTION_KEY, JSON.stringify(cookieData), 0, true, SameSiteOptions.None);
		}
	...
	getUserData(key) {
		if (!this.initialized) throw createBrowserAuthError(uninitializedPublicClientApplication);
		return this.memoryStorage.getItem(key);
	}
	async decryptData(key, data, correlationId) {
		if (!this.initialized || !this.encryptionCookie) throw createBrowserAuthError(uninitializedPublicClientApplication);
	...
	*/
	clear() {
		...
		getAccountKeys(this).forEach((key) => this.removeItem(key));
		...
		this.getKeys().forEach((cacheKey) => {
			if (cacheKey.startsWith("msal") || cacheKey.indexOf(this.clientId) !== -1) this.removeItem(cacheKey);
		});
	}
	/**
	...
	*/
	async importExistingCache(correlationId) {
		...
		if (accountKeys.length) this.setItem(getAccountKeysCacheKey(), JSON.stringify(accountKeys));
		else this.removeItem(getAccountKeysCacheKey());
		const tokenKeys = getTokenKeys(this.clientId, this);
		...
		if (tokenKeys.idToken.length || tokenKeys.accessToken.length || tokenKeys.refreshToken.length) this.setItem(getTokenKeysCacheKey(this.clientId), JSON.stringify(tokenKeys));
		else this.removeItem(getTokenKeysCacheKey(this.clientId));
	}
	/**
	...
	*/
	getContext(key) {
		let context = "";
		if (key.includes(this.clientId)) context = this.clientId;
		return context;
	}
	updateCache(event) {
...
//#endregion
//#region node_modules/@azure/msal-browser/dist/utils/Helpers.mjs
/*! @azure/msal-browser v4.30.0 2026-03-18 */
/**
...
var BrowserCacheManager = class extends CacheManager {
	...
	*/
	async updateOldEntry(key, correlationId) {
		...
		if ((isAccessTokenEntity(decryptedData) || isRefreshTokenEntity(decryptedData)) && decryptedData.expiresOn && isTokenExpired(decryptedData.expiresOn, 300)) {
			this.browserStorage.removeItem(key);
			this.performanceClient.incrementFields({ expiredCacheRemovedCount: 1 }, correlationId);
			return null;
		}
	...
	*/
	async removeAccountOldSchema(accountKey, rawObject, credentialSchema, correlationId) {
		const homeAccountId = (isEncrypted(rawObject) ? await this.browserStorage.decryptData(accountKey, rawObject, correlationId) : rawObject)?.homeAccountId;
		if (homeAccountId) {
	...
	getKMSIValues() {
		...
		for (const key of tokenKeys) {
			...
			if (rawValue) {
				...
				if (claims) kmsiMap[idToken.homeAccountId] = isKmsi(claims);
			}
		}
		return kmsiMap;
	}
	...
	*/
	async migrateIdTokens(credentialSchema, accountSchema, correlationId) {
		...
		const currentCredentialKeys = getTokenKeys(this.clientId, this.browserStorage, 2);
		...
		for (const idTokenKey of [...credentialKeysToMigrate.idToken]) {
			...
			const currentTokenHasSignInState = currentIdToken && Object.keys(extractTokenClaims(currentIdToken.secret, base64Decode) || {}).includes("signin_state");
			/**
			* Only migrate if:
	...
	*/
	trackVersionChanges(correlationId) {
		...
		if (previousVersion !== "4.30.0") this.setItem(VERSION_CACHE_KEY, version, correlationId);
	}
	/**
	...
	*/
	setItem(key, value, correlationId) {
		const tokenKeysCount = new Array(3).fill(0);
		const accessTokenKeys = [];
		const maxRetries = 20;
		for (let i = 0; i <= maxRetries; i++) try {
			this.browserStorage.setItem(key, value);
			if (i > 0) for (let schemaVersion = 0; schemaVersion <= 2; schemaVersion++) {
			...
			if (cacheError.errorCode === "cache_quota_exceeded" && i < maxRetries) {
				if (!accessTokenKeys.length) for (let i = 0; i <= 2; i++) if (key === getTokenKeysCacheKey(this.clientId, i)) {
	...
	*/
	async setUserData(key, value, correlationId, timestamp, kmsi) {
		...
		for (let i = 0; i <= maxRetries; i++) try {
			...
			const cacheError = createCacheError(e);
			if (cacheError.errorCode === "cache_quota_exceeded" && i < maxRetries) {
				if (!accessTokenKeys.length) for (let i = 0; i <= 2; i++) {
					const tokenKeys = this.getTokenKeys(i).accessToken;
					accessTokenKeys.push(...tokenKeys);
					tokenKeysCount[i] = tokenKeys.length;
				}
	...
	*/
	getAccount(accountKey, correlationId) {
		...
		this.performanceClient.addFields({ accountCachedBy: apiIdToName(parsedAccount.cachedByApiId) }, correlationId);
	...
	*/
	async setAccount(account, correlationId, kmsi, apiId) {
		...
		*/
		if (this.cacheConfig.cacheLocation === BrowserCacheLocation.LocalStorage && wasAdded) this.eventHandler.emitEvent(EventType.ACCOUNT_ADDED, void 0, AccountEntity.getAccountInfo(account));
	}
	/**
	...
	*/
	...
	setAccountKeys(accountKeys, correlationId, schemaVersion = 2) {
		if (accountKeys.length === 0) this.removeItem(getAccountKeysCacheKey(schemaVersion));
		else this.setItem(getAccountKeysCacheKey(schemaVersion), JSON.stringify(accountKeys), correlationId);
	}
	/**
	...
	*/
	removeAccount(account, correlationId) {
		...
		*/
		if (this.cacheConfig.cacheLocation === BrowserCacheLocation.LocalStorage) this.eventHandler.emitEvent(EventType.ACCOUNT_REMOVED, void 0, account);
	}
	/**
	...
	* @param tokenKeys
	*/
	removeAccessTokenKeys(keys, correlationId, schemaVersion = 2) {
	...
	* @returns
	*/
	setTokenKeys(tokenKeys, correlationId, schemaVersion = 2) {
		if (tokenKeys.idToken.length === 0 && tokenKeys.accessToken.length === 0 && tokenKeys.refreshToken.length === 0) {
			...
			return;
		} else this.setItem(getTokenKeysCacheKey(this.clientId, schemaVersion), JSON.stringify(tokenKeys), correlationId);
	}
	/**
	...
	*/
	clear(correlationId) {
		...
		this.temporaryCacheStorage.getKeys().forEach((cacheKey) => {
			if (cacheKey.indexOf("msal") !== -1 || cacheKey.indexOf(this.clientId) !== -1) this.removeTemporaryItem(cacheKey);
		});
		this.browserStorage.getKeys().forEach((cacheKey) => {
			if (cacheKey.indexOf("msal") !== -1 || cacheKey.indexOf(this.clientId) !== -1) this.browserStorage.removeItem(cacheKey);
		});
	...
	*/
	generateCacheKey(key) {
		if (StringUtils.startsWith(key, "msal")) return key;
	...
	*/
	getCachedRequest() {
		...
		try {
			...
			if (encodedVerifier) verifier = base64Decode(encodedVerifier);
		} catch (e) {
	...
	*/
	...
	setInteractionInProgress(inProgress, type = INTERACTION_TYPE.SIGNIN) {
		...
		else this.setTemporaryCache(key, JSON.stringify({
			clientId: this.clientId,
			type
		}), false);
	...
	*/
	async hydrateCache(result, request) {
		...
		const cacheRecord = {
			...
			accessToken: createAccessTokenEntity(result.account.homeAccountId, result.account.environment, result.accessToken, this.clientId, result.tenantId, result.scopes.join(" "), result.expiresOn ? toSecondsFromDate(result.expiresOn) : 0, result.extExpiresOn ? toSecondsFromDate(result.extExpiresOn) : 0, base64Decode, void 0, result.tokenType, void 0, request.sshKid, request.claims, claimsHash)
		};
...
function getStorageImplementation(clientId, cacheLocation, logger, performanceClient) {
	try {
		switch (cacheLocation) {
			case BrowserCacheLocation.LocalStorage: return new LocalStorage(clientId, logger, performanceClient);
...
var DEFAULT_BROWSER_CACHE_MANAGER = (clientId, logger, performanceClient, eventHandler) => {
	return new BrowserCacheManager(clientId, {
		...
		claimsBasedCachingEnabled: false
	}, DEFAULT_CRYPTO_IMPLEMENTATION, logger, performanceClient, eventHandler);
};
//#endregion
...
function validateRequestMethod(interactionRequest, protocolMode) {
	...
	if (protocolMode === ProtocolMode.EAR) {
		...
		if (httpMethod !== HttpMethod.POST) throw createClientConfigurationError(invalidRequestMethodForEAR);
	} else httpMethod = requestMethod || HttpMethod.GET;
	if (interactionRequest.authorizePostBodyParameters && httpMethod !== HttpMethod.POST) throw createClientConfigurationError(invalidAuthorizePostBodyParameters);
	return httpMethod;
}
//#endregion
...
var InteractionHandler = class {
	...
	*/
	async handleCodeResponseFromServer(authCodeResponse, request, apiId, validateNonce = true) {
		...
		return await invokeAsync(this.authModule.acquireToken.bind(this.authModule), PerformanceEvents.AuthClientAcquireToken, this.logger, this.performanceClient, request.correlationId)(this.authCodeRequest, apiId, authCodeResponse);
	}
	/**
...
function isFatalNativeAuthError(error) {
	if (error.ext && error.ext.status && error.ext.status === "DISABLED") return true;
	if (error.ext && error.ext.error && error.ext.error === INVALID_METHOD_ERROR) return true;
	switch (error.errorCode) {
		case contentError:
		case pageException: return true;
...
function createNativeAuthError(code, description, ext) {
	if (ext && ext.status) switch (ext.status) {
		...
		case UX_NOT_ALLOWED: return createInteractionRequiredAuthError(uxNotAllowed);
	}
...
//#endregion
//#region node_modules/@azure/msal-browser/dist/interaction_client/PlatformAuthInteractionClient.mjs
/*! @azure/msal-browser v4.30.0 2026-03-18 */
var PlatformAuthInteractionClient = class extends BaseInteractionClient {
	...
	*/
	async acquireToken(request, cacheLookupPolicy) {
		...
		try {
			...
			if (e instanceof NativeAuthError) serverTelemetryManager.setNativeBrokerErrorCode(e.errorCode);
	...
	*/
	async acquireTokensFromCache(nativeAccountId, request) {
		...
		const account = this.browserStorage.getBaseAccountInfo({ nativeAccountId }, this.correlationId);
	...
	async acquireTokenRedirect(request, rootMeasurement) {
		...
		try {
			...
			if (e instanceof NativeAuthError) {
				this.initializeServerTelemetryManager(this.apiId).setNativeBrokerErrorCode(e.errorCode);
	...
	*/
	async handleRedirectPromise(performanceClient, correlationId) {
		...
		try {
			...
			this.initializeServerTelemetryManager(this.apiId).clearNativeBrokerErrorCode();
			if (performanceClient && this.correlationId) this.performanceClient.addFields({ isNativeBroker: true }, this.correlationId);
			return authResult;
		} catch (e) {
			throw e;
		}
	...
	*/
	async handleNativeResponse(response, request, reqTimestamp) {
		...
		const cachedhomeAccountId = this.browserStorage.getAccountInfoFilteredBy({ nativeAccountId: request.accountId }, this.correlationId)?.homeAccountId;
		...
		else if (homeAccountIdentifier !== cachedhomeAccountId && response.account.id !== request.accountId) throw createNativeAuthError(userSwitch);
	...
	*/
	async generateAuthenticationResult(response, request, idTokenClaims, accountEntity, authority, reqTimestamp) {
		...
		const accountInfo = updateAccountTenantProfileData(AccountEntity.getAccountInfo(accountEntity), void 0, idTokenClaims, response.id_token);
		/**
	...
	*/
	async initializeNativeRequest(request) {
		...
		const mergedClaims = configClaims && configClaims.length ? addClientCapabilitiesToClaims$1(claims, configClaims) : claims;
		const validatedRequest = {
			...remainingProperties,
			claims: mergedClaims,
...
//#endregion
//#region node_modules/@azure/msal-browser/dist/protocol/Authorize.mjs
/*! @azure/msal-browser v4.30.0 2026-03-18 */
var clientDataAccountTypeMapping = new Map([["e", "AAD"], ["m", "MSA"]]);
/**
...
function parseClientData(clientdata) {
	...
	try {
		const parts = decodeURIComponent(clientdata).split("|");
...
async function getStandardParameters(config, authority, request, logger, performanceClient) {
	...
	if (request.platformBroker) {
		...
		performanceClient.addFields({ isPlatformAuthorizeRequest: true }, request.correlationId);
		if (request.authenticationScheme === AuthenticationScheme.POP) {
			const cryptoOps = new CryptoOps(logger, performanceClient);
			const popTokenGenerator = new PopTokenGenerator(cryptoOps);
			let reqCnfData;
			if (!request.popKid) reqCnfData = (await invokeAsync(popTokenGenerator.generateCnf.bind(popTokenGenerator), PerformanceEvents.PopTokenGenerateCnf, logger, performanceClient, request.correlationId)(request, logger)).reqCnfString;
...
async function getAuthCodeRequestUrl(config, authority, request, logger, performanceClient) {
	if (!request.codeChallenge) throw createClientConfigurationError(pkceParamsMissing);
...
async function getEARForm(frame, config, authority, request, logger, performanceClient) {
	if (!request.earJwk) throw createBrowserAuthError(earJwkEmpty);
	...
	return createForm(frame, getAuthorizeUrl(authority, queryParams, config.auth.encodeExtraQueryParams, request.extraQueryParameters), parameters);
}
/**
...
async function handleResponsePlatformBroker(request, accountId, apiId, config, browserStorage, nativeStorage, eventHandler, logger, performanceClient, platformAuthProvider) {
	...
	if (!platformAuthProvider) throw createBrowserAuthError(nativeConnectionNotEstablished);
...
async function handleResponseCode(request, response, codeVerifier, apiId, config, authClient, browserStorage, nativeStorage, eventHandler, logger, performanceClient, platformAuthProvider) {
	...
	if (response.accountId) return invokeAsync(handleResponsePlatformBroker, PerformanceEvents.HandleResponsePlatformBroker, logger, performanceClient, request.correlationId)(request, response.accountId, apiId, config, browserStorage, nativeStorage, eventHandler, logger, performanceClient, platformAuthProvider);
	const interactionHandler = new InteractionHandler(authClient, browserStorage, {
		...request,
		code: response.code || "",
		codeVerifier
	}, logger, performanceClient);
	return await invokeAsync(interactionHandler.handleCodeResponse.bind(interactionHandler), PerformanceEvents.HandleCodeResponse, logger, performanceClient, request.correlationId)(response, request, apiId);
}
/**
...
async function handleResponseEAR(request, response, apiId, config, authority, browserStorage, nativeStorage, eventHandler, logger, performanceClient, platformAuthProvider) {
	...
	if (!response.ear_jwe) throw createBrowserAuthError(earJweEmpty);
	if (!request.earJwk) throw createBrowserAuthError(earJwkEmpty);
	...
	return await invokeAsync(responseHandler.handleServerTokenResponse.bind(responseHandler), PerformanceEvents.HandleServerTokenResponse, logger, performanceClient, request.correlationId)(decryptedData, authority, nowSeconds(), request, apiId, additionalData, void 0, void 0, void 0, void 0);
}
//#endregion
...
//#endregion
//#region node_modules/@azure/msal-browser/dist/broker/nativeBroker/PlatformAuthExtensionHandler.mjs
/*! @azure/msal-browser v4.30.0 2026-03-18 */
var PlatformAuthExtensionHandler = class PlatformAuthExtensionHandler {
	...
	*/
	onWindowMessage(event) {
		...
		if (!request.channel || request.channel !== PlatformAuthConstants.CHANNEL_ID) return;
...
//#endregion
//#region node_modules/@azure/msal-browser/dist/broker/nativeBroker/PlatformAuthDOMHandler.mjs
/*! @azure/msal-browser v4.30.0 2026-03-18 */
var PlatformAuthDOMHandler = class PlatformAuthDOMHandler {
	...
	static async createProvider(logger, performanceClient, correlationId) {
		...
		if (window.navigator?.platformAuthentication) {
			if ((await window.navigator.platformAuthentication.getSupportedContracts(PlatformAuthConstants.MICROSOFT_ENTRA_BROKERID))?.includes(PlatformAuthConstants.PLATFORM_DOM_APIS)) {
	...
	*/
	async sendMessage(request) {
		...
		try {
			...
			const response = await window.navigator.platformAuthentication.executeGetToken(platformDOMRequest);
			return this.validatePlatformBrokerResponse(response);
		} catch (e) {
	...
	getDOMExtraParams(extraParameters) {
		try {
			...
			for (const [key, value] of Object.entries(extraParameters)) {
				if (!value) continue;
				if (typeof value === "object") stringifiedProperties[key] = JSON.stringify(value);
				else stringifiedProperties[key] = String(value);
			}
...
//#endregion
//#region node_modules/@azure/msal-browser/dist/broker/nativeBroker/PlatformAuthProvider.mjs
/*! @azure/msal-browser v4.30.0 2026-03-18 */
/**
...
async function getPlatformAuthProvider(logger, performanceClient, correlationId, nativeBrokerHandshakeTimeout, enablePlatformBrokerDOMSupport) {
	...
	try {
		if (enablePlatformBrokerDOMSupport) platformAuthProvider = await PlatformAuthDOMHandler.createProvider(logger, performanceClient, correlationId);
		if (!platformAuthProvider) {
			logger.trace("Platform auth via DOM API not available, checking for extension");
			platformAuthProvider = await PlatformAuthExtensionHandler.createProvider(logger, nativeBrokerHandshakeTimeout || 2e3, performanceClient);
		}
	} catch (e) {
...
function isPlatformAuthAllowed(config, logger, platformAuthProvider, authenticationScheme) {
	...
	if (!config.system.allowPlatformBroker && config.system.allowPlatformBrokerWithDOM) throw createClientConfigurationError(invalidPlatformBrokerConfiguration);
...
var PopupClient = class extends StandardInteractionClient {
	...
	*/
	async acquireTokenPopupAsync(request, popupParams, pkceCodes) {
		...
		if (popupParams.popup) preconnect(validRequest.authority);
		...
		if (this.config.auth.protocolMode === ProtocolMode.EAR) return this.executeEarFlow(validRequest, popupParams, pkceCodes);
	...
	*/
	async executeCodeFlow(request, popupParams, pkceCodes) {
		...
		try {
			...
			if (popupRequest.httpMethod === HttpMethod.POST) return await this.executeCodeFlowWithPost(popupRequest, popupParams, authClient, pkce.verifier);
			else {
	...
	*/
	async executeEarFlow(request, popupParams, pkceCodes) {
		...
		(await getEARForm(popupWindow.document, this.config, discoveredAuthority, popupRequest, this.logger, this.performanceClient)).submit();
	...
	async executeCodeFlowWithPost(request, popupParams, authClient, pkceVerifier) {
		...
		const popupWindow = popupParams.popup || this.openPopup("about:blank", popupParams);
		(await getCodeForm(popupWindow.document, this.config, discoveredAuthority, request, this.logger, this.performanceClient)).submit();
		const responseString = await invokeAsync(this.monitorPopupForHash.bind(this), PerformanceEvents.SilentHandlerMonitorIframeForHash, this.logger, this.performanceClient, correlationId)(popupWindow, popupParams.popupWindowParent);
		const serverParams = invoke(deserializeResponse, PerformanceEvents.DeserializeResponse, this.logger, this.performanceClient, this.correlationId)(responseString, this.config.auth.OIDCOptions.serverResponseType, this.logger);
...
var RedirectClient = class extends StandardInteractionClient {
	...
	*/
	async acquireToken(request) {
		...
		try {
			if (this.config.auth.protocolMode === ProtocolMode.EAR) await this.executeEarFlow(validRequest);
			else await this.executeCodeFlow(validRequest, request.onRedirectNavigate);
		} catch (e) {
			if (e instanceof AuthError) e.setCorrelationId(this.correlationId);
	...
	*/
	async executeCodeFlow(request, onRedirectNavigate) {
		...
		try {
			if (redirectRequest.httpMethod === HttpMethod.POST) return await this.executeCodeFlowWithPost(redirectRequest);
			else {
				const authClient = await invokeAsync(this.createAuthCodeClient.bind(this), PerformanceEvents.StandardInteractionClientCreateAuthCodeClient, this.logger, this.performanceClient, this.correlationId)({
					serverTelemetryManager,
	...
	*/
	async executeEarFlow(request) {
		...
		this.browserStorage.cacheAuthorizeRequest(redirectRequest, pkceCodes.verifier);
		(await getEARForm(document, this.config, discoveredAuthority, redirectRequest, this.logger, this.performanceClient)).submit();
		return new Promise((resolve, reject) => {
			setTimeout(() => {
				reject(createBrowserAuthError(timedOut, "failed_to_redirect"));
	...
	*/
	async executeCodeFlowWithPost(request) {
		...
		(await getCodeForm(document, this.config, discoveredAuthority, request, this.logger, this.performanceClient)).submit();
		return new Promise((resolve, reject) => {
			setTimeout(() => {
				reject(createBrowserAuthError(timedOut, "failed_to_redirect"));
			}, this.config.system.redirectNavigationTimeout);
	...
	*/
	async handleRedirectPromise(hash = "", request, pkceVerifier, parentMeasurement) {
		...
		try {
			...
			if (normalizeUrlForComparison(loginRequestUrl) === normalizeUrlForComparison(window.location.href) && this.config.auth.navigateToLoginRequestUrl) {
	...
	*/
	async handleResponse(serverParams, request, codeVerifier, serverTelemetryManager) {
		if (!serverParams.state) throw createBrowserAuthError(noStateInHash);
	...
	async initiateAuthRequest(requestUrl, onRedirectNavigateRequest) {
		...
		if (requestUrl) {
			...
			if (typeof onRedirectNavigate === "function") {
				...
				if (onRedirectNavigate(requestUrl) !== false) {
	...
	async logout(logoutRequest) {
		...
		try {
			...
			if (logoutRequest && typeof logoutRequest.onRedirectNavigate === "function") if (logoutRequest.onRedirectNavigate(logoutUri) !== false) {
				...
				if (!this.browserStorage.getInteractionInProgress()) this.browserStorage.setInteractionInProgress(true, INTERACTION_TYPE.SIGNOUT);
...
async function initiateCodeFlowWithPost(config, authority, request, logger, performanceClient) {
	...
	(await getCodeForm(frame.contentDocument, config, authority, request, logger, performanceClient)).submit();
	return frame;
}
async function initiateEarRequest(config, authority, request, logger, performanceClient) {
	const frame = createHiddenIframe();
	if (!frame.contentDocument) throw "No document associated with iframe!";
	(await getEARForm(frame.contentDocument, config, authority, request, logger, performanceClient)).submit();
	return frame;
}
/**
...
var SilentIframeClient = class extends StandardInteractionClient {
	...
	*/
	async acquireToken(request) {
		...
		if (this.config.auth.protocolMode === ProtocolMode.EAR) return this.executeEarFlow(silentRequest);
	...
	*/
	async verifySso(request) {
		this.performanceClient.addQueueMeasurement(PerformanceEvents.SilentIframeClientAcquireToken, request.correlationId);
		const inputRequest = { ...request };
		if (!inputRequest.prompt) inputRequest.prompt = PromptValue.NONE;
		const silentRequest = await invokeAsync(this.initializeAuthorizationRequest.bind(this), PerformanceEvents.StandardInteractionClientInitializeAuthorizationRequest, this.logger, this.performanceClient, request.correlationId)(inputRequest, InteractionType.Silent);
	...
	*/
	async silentTokenHelper(authClient, request) {
		...
		let msalFrame;
		if (request.httpMethod === HttpMethod.POST) msalFrame = await invokeAsync(initiateCodeFlowWithPost, PerformanceEvents.SilentHandlerInitiateAuthRequest, this.logger, this.performanceClient, correlationId)(this.config, authClient.authority, silentRequest, this.logger, this.performanceClient);
		else {
...
var TokenCache = class {
	...
	*/
	async loadAccount(request, clientInfo, correlationId, authority, idTokenClaims) {
		...
		const cachedAccount = buildAccountToCache(this.storage, authority, homeAccountId, base64Decode, correlationId, idTokenClaims, clientInfo, authority.getPreferredCache(), claimsTenantId, void 0, void 0, this.logger);
	...
	*/
	async loadRefreshToken(response, homeAccountId, environment, correlationId, kmsi) {
		...
		const expiresOn = response.refresh_token_expires_in ? response.refresh_token_expires_in + nowSeconds() : void 0;
		this.performanceClient.addFields({ extRtExpiresOnSeconds: expiresOn }, correlationId);
		this.logger.verbose("TokenCache - loading refresh token");
		const refreshTokenEntity = createRefreshTokenEntity(homeAccountId, environment, response.refresh_token, this.config.auth.clientId, response.foci, void 0, expiresOn);
...
//#endregion
//#region node_modules/@azure/msal-browser/dist/utils/MsalFrameStatsUtils.mjs
/*! @azure/msal-browser v4.30.0 2026-03-18 */
/**
...
function collectInstanceStats(currentClientId, performanceEvent, logger) {
	const frameInstances = window.msal?.clientIds || [];
...
var StandardController = class StandardController {
	...
	*/
	async handleRedirectPromiseInternal(hash) {
		...
		if (this.browserStorage.getInteractionInProgress()?.type === INTERACTION_TYPE.SIGNOUT) {
		...
		try {
			if (useNative && this.platformAuthProvider) {
				...
				rootMeasurement.add({ isPlatformBrokerRequest: true });
		...
		return redirectResponse.then((result) => {
			if (result) {
				...
				rootMeasurement.end({ success: true }, void 0, result.account);
				this.verifySsoCapability(result.account, InteractionType.Redirect);
			} else if (rootMeasurement.event.errorCode) rootMeasurement.end({ success: false }, void 0);
	...
	*/
	async acquireTokenRedirect(request) {
		...
		atrMeasurement.add({ scenarioId: request.scenarioId });
		...
		if (onRedirectNavigateCb) request.onRedirectNavigate = (url) => {
			...
			atrMeasurement.add({ navigateCallbackResult: navigate !== false });
		...
		try {
			...
			let result;
			if (this.platformAuthProvider && this.canUsePlatformBroker(request)) result = new PlatformAuthInteractionClient(this.config, this.browserStorage, this.browserCrypto, this.logger, this.eventHandler, this.navigationClient, ApiId.acquireTokenRedirect, this.performanceClient, this.platformAuthProvider, this.getNativeAccountId(request), this.nativeInternalStorage, correlationId).acquireTokenRedirect(request, atrMeasurement).catch((e) => {
				atrMeasurement.add({
			...
			this.browserStorage.resetRequestCache();
			if (atrMeasurement.event.status === 2) this.performanceClient.startMeasurement(PerformanceEvents.AcquireTokenRedirect, correlationId).end({ success: false }, e, request.account);
			else atrMeasurement.end({ success: false }, e, request.account);
	...
	*/
	acquireTokenPopup(request) {
		...
		if (this.canUsePlatformBroker(request)) {
			atPopupMeasurement.add({ isPlatformBrokerRequest: true });
			result = this.acquireTokenNative({
				...request,
				correlationId
			}, ApiId.acquireTokenPopup).then((response) => {
				atPopupMeasurement.end({ success: true }, void 0, response.account);
				return response;
			}).catch((e) => {
				...
				if (e instanceof NativeAuthError && isFatalNativeAuthError(e)) {
					this.platformAuthProvider = void 0;
					return this.createPopupClient(correlationId).acquireToken(request, pkce);
				} else if (e instanceof InteractionRequiredAuthError) {
		...
		} else result = this.createPopupClient(correlationId).acquireToken(request, pkce);
		return result.then((result) => {
			...
			atPopupMeasurement.end({ success: false }, e, request.account);
			return Promise.reject(e);
		}).finally(async () => {
			this.browserStorage.setInteractionInProgress(false);
	...
	trackStateChangeWithMeasurement(event) {
		...
		if (event.type === "visibilitychange") {
			...
			measurement.increment({ onlineStatusChangeCount: 1 });
		} else if (event.type === "offline") {
	...
	*/
	verifySsoCapability(account, parentApi) {
		if (!this.config.auth.verifySSO) return;
		...
		ssoCapableMeasurement.add({ parentApi });
		...
		setTimeout(() => {
			...
			this.createSilentIframeClient(correlationId).verifySso(ssoVerificationRequest).then((success) => {
	...
	*/
	async ssoSilent(request) {
		...
		if (this.canUsePlatformBroker(validRequest)) {
			this.ssoSilentMeasurement?.add({ isPlatformBrokerRequest: true });
		...
		return result.then((response) => {
			...
			this.ssoSilentMeasurement?.end({ success: false }, e, request.account);
			throw e;
		}).finally(() => {
	...
	*/
	async acquireTokenByCode(request) {
		...
		try {
			if (request.code && request.nativeAccountId) throw createBrowserAuthError(spaCodeAndNativeAccountIdPresent);
			...
			} else if (request.nativeAccountId) if (this.canUsePlatformBroker(request, request.nativeAccountId)) {
				atbcMeasurement.add({ isPlatformBrokerRequest: true });
				const result = await this.acquireTokenNative({
					...request,
					correlationId
				}, ApiId.acquireTokenByCode, request.nativeAccountId).catch((e) => {
					if (e instanceof NativeAuthError && isFatalNativeAuthError(e)) this.platformAuthProvider = void 0;
					atbcMeasurement.add({
						brokerErrorName: e.name,
						brokerErrorCode: e.errorCode
					});
				...
				atbcMeasurement.end({ success: true }, void 0, result.account);
				return result;
	...
	*/
	logoutPopup(logoutRequest) {
		try {
			...
			this.browserStorage.setInteractionInProgress(true, INTERACTION_TYPE.SIGNOUT);
			return this.createPopupClient(correlationId).logout(logoutRequest).finally(() => {
				this.browserStorage.setInteractionInProgress(false);
			});
		} catch (e) {
			return Promise.reject(e);
	...
	*/
	async acquireTokenNative(request, apiId, accountId, cacheLookupPolicy) {
		...
		if (!this.platformAuthProvider) throw createBrowserAuthError(nativeConnectionNotEstablished);
		return new PlatformAuthInteractionClient(this.config, this.browserStorage, this.browserCrypto, this.logger, this.eventHandler, this.navigationClient, apiId, this.performanceClient, this.platformAuthProvider, accountId || this.getNativeAccountId(request), this.nativeInternalStorage, request.correlationId).acquireToken(request, cacheLookupPolicy);
	}
	/**
	...
	*/
	async acquireTokenSilent(request) {
		...
		return this.acquireTokenSilentDeduped(request, account, correlationId).then((result) => {
			...
			if (error instanceof AuthError) error.setCorrelationId(correlationId);
			atsMeasurement.end({ success: false }, error, account);
			throw error;
		});
	}
	/**
	...
	*/
	async acquireTokenSilentAsync(request, account) {
		...
		return this.acquireTokenSilentNoIframe(silentRequest, cacheLookupPolicy).catch(async (refreshTokenError) => {
			if (checkIfRefreshTokenErrorCanBeResolvedSilently(refreshTokenError, cacheLookupPolicy)) {
			...
			this.performanceClient.addFields({ fromCache: response.fromCache }, request.correlationId);
			return response;
		}).catch((tokenRenewalError) => {
	...
	*/
	async acquireTokenSilentNoIframe(silentRequest, cacheLookupPolicy) {
		if (isPlatformAuthAllowed(this.config, this.logger, this.platformAuthProvider, silentRequest.authenticationScheme) && silentRequest.account.nativeAccountId) {
			...
			if (cacheLookupPolicy === CacheLookupPolicy.AccessToken) this.logger.verbose("acquireTokenSilent - cache lookup policy set to AccessToken, attempting to acquire token from local cache");
	...
	*/
	...
	logMultipleInstances(performanceEvent) {
		...
		if (window.msal.clientIds.length > 0) this.logger.verbose("There is already an instance of MSAL.js in the window.");
...
var NestedAppAuthController = class NestedAppAuthController {
	...
	*/
	async acquireTokenSilentInternal(request) {
		...
		try {
			...
			ssoSilentMeasurement?.end({ success: false }, e, request.account);
			throw error;
		}
	}
	/**
	...
	*/
	async acquireTokenFromCache(request) {
		...
		let result = null;
		if (!request.cacheLookupPolicy) request.cacheLookupPolicy = CacheLookupPolicy.Default;
		...
		if (result) {
			...
			atsMeasurement.end({ success: true }, void 0, result.account);
			return result;
		}
		...
		atsMeasurement.end({ success: false }, void 0, request.account);
		return null;
	}
	/**
	*
	...
	*/
	async acquireTokenFromCacheInternal(request) {
		...
		if (accountContext) currentAccount = getAccount(accountContext, this.logger, this.browserStorage, correlationId);
		if (!currentAccount) {
...
async function createNestablePublicClientApplication(configuration) {
	...
	if (nestedAppAuth.isAvailable()) {
		const nestablePCA = new PublicClientApplication(configuration, new NestedAppAuthController(nestedAppAuth));
...
var PublicClientNext = class PublicClientNext {
	...
	*/
	constructor(configuration, controller) {
		...
		else this.controller = new UnknownOperatingContextController(new UnknownOperatingContext(configuration));
	}
	/**
	* Initializer function to perform async startup tasks such as connecting to WAM extension
	*/
...
var SignedHttpRequest = class {
	constructor(shrParameters, shrOptions) {
		this.logger = new Logger(shrOptions && shrOptions.loggerOptions || {}, name, version);
	...
	*/
	async removeKeys(publicKeyThumbprint) {
		return this.cryptoOps.removeTokenBindingKey(publicKeyThumbprint).then(() => true).catch((error) => {
			if (error instanceof ClientAuthError && error.errorCode === "binding_key_not_removed") return false;
			throw error;
		});
	}
};
...
function getPerfMeasurementModule() {
	...
	try {
		...
		if (Number(perfEnabled) === 1) return import("./BrowserPerformanceMeasurement-CtWHITno.js").then((n) => n.n);
	} catch (e) {}
}
/**
